...
let scoreWhenPoints = (current, winner) =>
  ...
  switch score {
    ...
    | Deuce => "40/40 : Deuce"
    | Forty(forty) => (forty.player == PlayerOne) ?("Player 1" ++" : 40/"++ string_of_point(forty.otherPlayerPoint)++" : Player 2") : ("Player 1 : " ++ string_of_point(forty.otherPlayerPoint) ++ "/40 : Player 2")
